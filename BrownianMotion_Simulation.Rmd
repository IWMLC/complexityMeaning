...
```{r, width = 4, height = 5}
simulation.df <- as.data.frame(t(simulation)) # the t() function is transposing the rows and columns
...
density.plot <- ggplot(simulation.df.long, aes(x = value)) + 
 ...
 geom_jitter(data = simulation.df.long, aes(x = value, y = 0), 
             size = 1, height = 0.1, width = 0) + # add some jitter to prevent overplotting of dots
 facet_wrap(~ language) +
