---
title: 'Appendix I: Simulation Study with Normal Distribution and Location Shift'
author: "Chris Bentz"
...
set.seed(1) # set the seed for random number generation in order to get the same result when the code is re-run
langA.values <- rnorm(n) # generate values for language A
langB.values <- rnorm(n) # generate values for language B
langC.values <- rnorm(n) + 1 # generate values for language C with location shift (1 standard deviation)

value <- c(langA.values, langB.values, langC.values) # concatenate to get long format data frame
...
density.plot <- ggplot(simulation.df, aes(x = value)) + 
 # geom_histogram(aes(y = ..density..), colour = "white", fill = "light grey",
                # binwidth = 0.1) + # histograms could be added as well here (or instead of the density distributions)
 geom_density(alpha = .2, fill = "grey", color = "darkgrey") +
 geom_jitter(data = simulation.df, aes(x = value, y = 0), 
             size = 1, height = 0.03, width = 0) + # add some jitter to prevent overplotting of dots
 facet_wrap(~ language) +
...
stats.df.sorted[, -1] <- round(stats.df.sorted[, -1], 2) # round values to two decimal places,  the "-1" excludes column 1
